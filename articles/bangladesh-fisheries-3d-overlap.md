...
deepest `depth_min` and the shallowest `depth_max` win. Passing
`seafloor` as a second `depth_max` is what keeps a species or fishery
...
species_proj <- sf::st_transform(bangladesh_species, bangladesh_crs)

species_rasters <- lapply(seq_len(nrow(species_proj)), function(i) {
  vect_to_envelope(
    polygon   = species_proj[i, ],
    template  = study_grid,
    depth_min = species_proj$depth_min_sel[i],
    depth_max = list(species_proj$depth_max_sel[i], seafloor)
  )
})
names(species_rasters) <- species_proj$sci_name
```
...
fisheries_proj <- sf::st_transform(fisheries, bangladesh_crs)

fishery_rasters <- lapply(seq_len(nrow(fisheries_proj)), function(i) {
  vect_to_envelope(
    polygon   = fisheries_proj[i, ],
    template  = study_grid,
    depth_min = fisheries_proj$depth_min[i],
    depth_max = list(fisheries_proj$depth_max[i], seafloor)
  )
})
names(fishery_rasters) <- fisheries_proj$fishery_type
```
...
for (i in seq_len(n_pairs)) {
  ...
  ov <- calc_volume_overlap(
    x = species_rasters[[sp]],
    y = fishery_rasters[[fi]]
  )
...
those zeros stay transparent and the map looks as before; the difference
is in the numbers, which now separate “surveyed, no overlap” from “no
data”.
...
fishery_richness_plots <- lapply(cr_species, function(sp) {
  ...
  # Stack of TRUE/FALSE/NA overlap rasters, summed across layers, gives the
  # number of fisheries that overlap the species in 3D at each cell.
  overlap_stack <- terra::rast(lapply(fishery_rasters, function(fi_rast) {
    intersects_3d(sp_rast, fi_rast)
  }))

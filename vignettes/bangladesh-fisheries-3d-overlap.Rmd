...
resolution. `load_bathymetry()` loads and standardises the raster (positive
...
> **Note:** This step requires the reference CSV from the original project
> (`previous_projects/bangladesh-fisheries-3d-overlap/`), which is not
> included in this package. It is provided for reproducibility when the
> original project data is available; the analysis above (Steps 1–3) and the
> visualisation below (Step 5) run without it.

```{r compare-original}
# Load reference results from original analysis (per species-fishery pair)
reference <- read.csv(
  here("previous_projects", "bangladesh-fisheries-3d-overlap",
